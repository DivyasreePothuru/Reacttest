...
function App() {
  const data = [
    {name:"Divya" , password:"abc"},
    {name:"shabnam" , password:"def"},
    {name:"julian" , password:"ghi"}
  ]

 const [user, setuser] = useState()
 const [users,setusers] = useState(data)
 const navigate = useNavigate()
 const notify = (user) => toast(`Welcome ${user}`);
...
useEffect( () => {
  ...
  <div className="flex-1">
    <a className="btn btn-ghost normal-case text-xl logo">DS</a>
  </div>
